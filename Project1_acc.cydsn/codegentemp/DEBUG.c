...
static void DEBUG_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void DEBUG_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            hsiomSel[DEBUG_RX_SDA_MOSI_PIN_INDEX] = DEBUG_HSIOM_I2C_SEL;
            hsiomSel[DEBUG_TX_SCL_MISO_PIN_INDEX] = DEBUG_HSIOM_I2C_SEL;

            pinsDm[DEBUG_RX_SDA_MOSI_PIN_INDEX] = DEBUG_PIN_DM_OD_LO;
            pinsDm[DEBUG_TX_SCL_MISO_PIN_INDEX] = DEBUG_PIN_DM_OD_LO;
        }
    #if (!DEBUG_CY_SCBIP_V1)
        else if (DEBUG_SCB_MODE_SPI == mode)
        {
            hsiomSel[DEBUG_RX_SDA_MOSI_PIN_INDEX] = DEBUG_HSIOM_SPI_SEL;
            hsiomSel[DEBUG_TX_SCL_MISO_PIN_INDEX] = DEBUG_HSIOM_SPI_SEL;
            hsiomSel[DEBUG_CTS_SCLK_PIN_INDEX] = DEBUG_HSIOM_SPI_SEL;

            if (DEBUG_SPI_SLAVE == subMode)
            {
                /* Slave */
                pinsDm[DEBUG_RX_SDA_MOSI_PIN_INDEX] = DEBUG_PIN_DM_DIG_HIZ;
                pinsDm[DEBUG_TX_SCL_MISO_PIN_INDEX] = DEBUG_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= DEBUG_TX_SCL_MISO_PIN_MASK;
            #endif /* (DEBUG_TX_SCL_MISO_PIN) */
            }
            else /* (Master) */
            {
                pinsDm[DEBUG_RX_SDA_MOSI_PIN_INDEX] = DEBUG_PIN_DM_STRONG;
                pinsDm[DEBUG_TX_SCL_MISO_PIN_INDEX] = DEBUG_PIN_DM_DIG_HIZ;
                ...
                pinsInBuf |= DEBUG_RX_SDA_MOSI_PIN_MASK;
            #endif /* (DEBUG_RX_SDA_MOSI_PIN) */

             #if (DEBUG_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= DEBUG_RX_WAKE_SDA_MOSI_PIN_MASK;
        ...
        else /* UART */
        {
            ...
            {
                /* SmartCard */
                hsiomSel[DEBUG_TX_SCL_MISO_PIN_INDEX] = DEBUG_HSIOM_UART_SEL;
                pinsDm  [DEBUG_TX_SCL_MISO_PIN_INDEX] = DEBUG_PIN_DM_OD_LO;
            }
            else /* Standard or IrDA */
            {
                ...
                {
                    hsiomSel[DEBUG_RX_SDA_MOSI_PIN_INDEX] = DEBUG_HSIOM_UART_SEL;
                    pinsDm  [DEBUG_RX_SDA_MOSI_PIN_INDEX] = DEBUG_PIN_DM_DIG_HIZ;
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        DEBUG_SET_HSIOM_SEL(DEBUG_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[DEBUG_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        DEBUG_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[DEBUG_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        DEBUG_SET_INP_DIS(DEBUG_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     DEBUG_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & DEBUG_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        DEBUG_SET_INCFG_TYPE(DEBUG_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        DEBUG_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
        ...
        DEBUG_SET_HSIOM_SEL(DEBUG_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       DEBUG_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[DEBUG_RX_SDA_MOSI_PIN_INDEX]);

        DEBUG_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[DEBUG_RX_SDA_MOSI_PIN_INDEX]);

    #if (!DEBUG_CY_SCBIP_V1)
        DEBUG_SET_INP_DIS(DEBUG_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                     DEBUG_uart_rx_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & DEBUG_RX_SDA_MOSI_PIN_MASK)));
    #endif /* (!DEBUG_CY_SCBIP_V1) */
    #endif /* (DEBUG_RX_SDA_MOSI_PIN) */

    #if (DEBUG_TX_SCL_MISO_PIN)
        DEBUG_SET_HSIOM_SEL(DEBUG_TX_SCL_MISO_HSIOM_REG,
                                       DEBUG_TX_SCL_MISO_HSIOM_MASK,
                                       DEBUG_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[DEBUG_TX_SCL_MISO_PIN_INDEX]);

        DEBUG_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[DEBUG_TX_SCL_MISO_PIN_INDEX]);

    #if (!DEBUG_CY_SCBIP_V1)
        DEBUG_SET_INP_DIS(DEBUG_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     DEBUG_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & DEBUG_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!DEBUG_CY_SCBIP_V1) */
    #endif /* (DEBUG_RX_SDA_MOSI_PIN) */
